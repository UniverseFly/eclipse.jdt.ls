...
public final class NewCompletionProposalRequestor extends CompletionRequestor {
	...
	// Source code of the compilation unit
	 ...
	 */
	@Deprecated
	public NewCompletionProposalRequestor(ICompilationUnit aUnit, int offset) throws JavaModelException {
		this(aUnit, offset, JavaLanguageServerPlugin.getPreferencesManager());
	}

	public NewCompletionProposalRequestor(ICompilationUnit aUnit, int offset, PreferenceManager preferenceManager) throws JavaModelException {
		this.unit = aUnit;
	...
	public List<NewCompletionItem> getCompletionItems() {
		//Sort the results by relevance 1st
		proposals.sort(new ProposalComparator(proposals.size()));
		List<NewCompletionItem> completionItems = new ArrayList<>(proposals.size());
		...
		for (int i = 0; i < proposals.size(); i++) {
			...
			try {
				var item = toCompletionItem(proposal, i);
